...
import { parseRegionConfig, dispatchToRegion, VARIANT_NAME, type RegionConfig } from '../lib/sagemaker.js';
 ...
 * scaler). AWS exposes no "capacity availability" API for SageMaker — a
 * region's g7e capacity is only proven by real provisioning attempts. This
 * sentinel therefore watches live evidence from every candidate region's
 * endpoint + scaling activities and elects the active region automatically:
 *
 ...
 *
 * Election (candidate order in SAGEMAKER_REGIONS = failback priority):
 *   1. Active is HEALTHY/PROVISIONING -> only flip to a HIGHER-priority
 *      region that is healthy-PROVEN (current >= 1 or a recent Successful
 *      activity); an idle-unproven region must not steal traffic and re-enter
 *      the capacity lottery.
 *   2. Active is DROUGHT/FAILED -> flip to the first HEALTHY region
 *      (idle-unproven counts — the flip itself is the probe).
 *   3. No healthy candidate -> stay (every region's endpoint keeps retrying
 *      for free).
 *
 * Flips are rate-limited by FLIP_COOLDOWN_SECONDS. On each run, while the
 * active region is HEALTHY, stranded tasks (IN_PROGRESS with a task token
 * dispatched to a region other than the active one) are re-dispatched to the
 * active region; their parked state-machine tokens resume normally on the
 * new region's success callback, and late duplicate callbacks from the
...
async function describeRegion(conf: RegionConfig): Promise<EndpointFacts> {
...
export async function handler(): Promise<{
  ...
  const activeParam = requireEnv('ACTIVE_REGION_PARAM');
  const lastFlipParam = requireEnv('LAST_FLIP_PARAM');
  const cooldownSeconds = Number(process.env.FLIP_COOLDOWN_SECONDS ?? '300');

  // Read the current election. Unknown/absent -> first configured region.
  const storedActive = await getParameterValue(activeParam);
  let activeRegion =
    storedActive && configs.some((c) => c.region === storedActive) ? storedActive : configs[0]?.region;
  if (!activeRegion) {
    throw new Error('No active region resolved (SAGEMAKER_REGIONS is empty)');
  }

  // Classify every candidate region, preserving configured order.
  ...
  for (const s of states) classes[s.conf.region] = s.class;

  // ---- election ----------------------------------------------------------
  let flipped = false;
  const activeIdx = states.findIndex((s) => s.conf.region === activeRegion);
  const activeClass = states[activeIdx]?.class ?? 'DROUGHT';

  let target: string | undefined;
  if (activeClass === 'HEALTHY' || activeClass === 'PROVISIONING') {
    // Failback only: a higher-priority (earlier) healthy-PROVEN region.
    target = states
      .slice(0, activeIdx)
      .find((s) => s.class === 'HEALTHY' && s.proven)
      ?.conf.region;
  } else {
    // Active region is dry/dead: first HEALTHY candidate (idle-unproven ok).
    target = states.find((s) => s.class === 'HEALTHY')?.conf.region;
  }

  if (target && target !== activeRegion) {
    ...
    if (withinCooldown) {
      ...
      console.log(
        `Flipped active region ${activeRegion} -> ${target} (active=${activeClass}, classes=${JSON.stringify(classes)})`,
      );
      activeRegion = target;
      flipped = true;
    }
  } else if (!target && activeClass !== 'HEALTHY' && activeClass !== 'PROVISIONING') {
    console.log(`Active region ${activeRegion} is ${activeClass} and no healthy candidate exists; staying`);
  }

  // ---- stranded-task rescue ---------------------------------------------
  let rescued = 0;
  const activeState = states.find((s) => s.conf.region === activeRegion);
  if (activeState && activeState.class === 'HEALTHY') {
    ...
    for (const task of stranded) {
      ...
      if (taskRegion === activeRegion) continue;
      try {
        ...
        await docClient.send(
          new UpdateCommand({
            ...
            ExpressionAttributeValues: { ':region': activeRegion, ':now': new Date().toISOString() },
            ConditionExpression: 'attribute_exists(pk)',
          }),
        );
        rescued += 1;
        console.log(`Re-dispatched stranded task ${task.task_id} from ${taskRegion} to ${activeRegion}`);
      } catch (err) {
        console.error(`Failed to re-dispatch stranded task ${task.task_id} to ${activeRegion}`, err);
      }
    }
  }

  return { active_region: activeRegion, flipped, classes, rescued };
}
